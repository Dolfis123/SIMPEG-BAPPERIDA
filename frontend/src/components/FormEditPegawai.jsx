...
// src/components/FormEditPegawai.jsx
...
import { updatePegawai } from "../api/pegawaiService";
import { Loader2 } from "lucide-react";

const FormEditPegawai = ({ onClose, onSuccess, initialData }) => {
  // State untuk form diisi dengan data awal dari pegawai yang dipilih
  const [formData, setFormData] = useState(initialData);

  // State untuk menyimpan data master (dropdown)
  ...
  // State error tidak lagi dibutuhkan untuk tampilan, tapi bagus untuk logging
  // const [error, setError] = useState('');

  // Efek ini berjalan saat komponen pertama kali menerima data pegawai
  useEffect(() => {
    // Fungsi aman untuk memformat tanggal ke format YYYY-MM-DD
    ...
    // Mengisi form dengan data awal dan memformat semua tanggal
    const formattedData = {
      ...initialData,
      tanggal_lahir: formatISODate(initialData.tanggal_lahir),
      tmt_pangkat_terakhir: formatISODate(initialData.tmt_pangkat_terakhir),
      tmt_kgb_terakhir: formatISODate(initialData.tmt_kgb_terakhir),
      tmt_cpns: formatISODate(initialData.tmt_cpns),
      tmt_pns: formatISODate(initialData.tmt_pns),
      tmt_jabatan_sekarang: formatISODate(initialData.tmt_jabatan_sekarang),
    };
    ...
    const fetchMasterData = async () => {
      try {
        ...
        // setError('Gagal memuat data referensi.'); // Bisa diganti alert juga jika perlu
        console.error("Gagal memuat data referensi.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    // setError(''); // Tidak perlu lagi
    try {
      // Memanggil fungsi updatePegawai dari service API
      await updatePegawai(formData.id, formData);
      // alert("Data pegawai berhasil diperbarui!");
      onSuccess(); // Memberitahu halaman utama untuk refresh data
      onClose(); // Menutup modal
    } catch (err) {
      // --- PERUBAHAN DI SINI ---
      // Menampilkan pesan error sebagai alert popup
      alert(err.toString());
    } finally {
  ...
  return (
    ...
    >
      {/* --- Kotak error di dalam form sudah dihapus --- */}
